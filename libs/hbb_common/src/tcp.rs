...
impl FramedStream {
    ...
    pub fn set_key(&mut self, key: Key) {
        if let Some(encryption) = self.2.as_mut() {
            encryption.push_key(key);
        } else {
...
#[cfg(test)]
mod restored_encryption_tests {
    use super::*;

    #[test]
    fn transport_layer_survives_session_key_installation() {
        sodiumoxide::init().unwrap();
        let transport = Key([7; secretbox::KEYBYTES]);
        let session = Key([9; secretbox::KEYBYTES]);
        let mut sender = Encrypt::new(transport.clone());
        let mut relay = Encrypt::new(transport);
        let mut receiver = Encrypt::new(session.clone());
        let mut authentication = BytesMut::from(sender.enc(b"certificate").as_slice());
        relay.dec(&mut authentication).unwrap();
        assert_eq!(authentication.as_ref(), b"certificate");
        sender.push_key(session);
        for payload in [b"frame one".as_slice(), b"frame two".as_slice()] {
            let mut frame = BytesMut::from(sender.enc(payload).as_slice());
            relay.dec(&mut frame).unwrap();
            receiver.dec(&mut frame).unwrap();
    ...
    fn modified_ciphertext_is_rejected() {
        ...
        let mut frame = BytesMut::from(sender.enc(b"certificate challenge").as_slice());
        frame[0] ^= 1;
        assert!(receiver.dec(&mut frame).is_err());
    }
}
