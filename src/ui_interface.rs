...
pub fn get_options() -> String {
    ...
    if option_env!("CNG_SERVER_ADDRESS").is_some_and(|server| !server.is_empty()) {
        let id_server = Config::get_rendezvous_server();
        let relay_server = crate::increase_port(&id_server, 1);
        m.insert("custom-rendezvous-server".into(), id_server.into());
        m.insert("relay-server".into(), relay_server.into());
    }
    if let Some(key) = option_env!("CNG_SERVER_PUBLIC_KEY").filter(|key| !key.is_empty()) {
